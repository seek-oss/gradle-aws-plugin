...
class UploadFiles extends Upload {
  ...
  private val failIfPrefixExists =
    lazyProperty[Boolean]("failIfPrefixExists", true)
  def failIfPrefixExists(v: Any): Unit = failIfPrefixExists.set(v)

  private val failIfObjectExists =
    lazyProperty[Boolean]("failIfObjectExists", false)
  def failIfObjectExists(v: Any): Unit = failIfObjectExists.set(v)

  private val cleanPrefixBeforeUpload =
    lazyProperty[Boolean]("cleanPrefixBeforeUpload", false)
  ...
  private def maybeFailIfPrefixExists(
      bucket: String,
      prefix: String): Kleisli[IO, AmazonS3, Unit] =
    maybeRun(
      failIfPrefixExists,
      exists(bucket, prefix),
      raiseError(s"Prefix '${prefix}' already exists in bucket ${bucket}"))

  private def maybeFailIfObjectExists(
      bucket: String,
      keys: List[String]): Kleisli[IO, AmazonS3, Unit] =
    maybeRun(failIfObjectExists,
             existsAny(bucket, keys),
             raiseError(
               s"Upload would overwrite one or more files in bucket ${bucket}"))

  private def maybeCleanPrefixBeforeUpload(
      bucket: String,
      prefix: String): Kleisli[IO, AmazonS3, Unit] =
    Kleisli { c =>
      cleanPrefixBeforeUpload.run.flatMap {
        ...
        case true =>
          if (prefix.isEmpty)
            raiseError(
              "No prefix specified to clean (and refusing to delete entire bucket)")
          else
            deleteAll(bucket, prefix).run(c)
      }
    }

  private def uploadAll(
      bucket: String,
      keyFileMap: Map[String, File],
  ...
  private def keyFileMap(files: FileCollection,
                         prefix: String): Map[String, File] = {
